...
const m = 60, I = (n) => (t) => t % n, c = I(m), f = (n, t) => (i) => c(n + i * t), M = (n, t) => c(m + n - t), u = (n) => (t, i) => !i || t.getHours() === i.getHours() ? t : r(t, n), l = u(0), L = u(m - 1);
class T {
  constructor(t) {
  ...
  data(t) {
    const [i] = this.range(t), s = f(i, this.step), h = (o) => this.toListItem(s(o)), e = x(0, this.countFromMin(t)).map(h);
  ...
  addLast(t, i) {
    return this.insertUndividedMax && this.isLastMissing(i) && t.push(this.toListItem(this.lastMinute(i))), t;
  }
  addMissing(t, i) {
  ...
  countFromMin(t) {
    ...
    return Math.floor(M(s, i) / this.step) + 1;
  }
  isMissing(t) {
    return t ? this.selectedIndex(t) !== this.divideByStep(t) : !1;
  }
  isLastMissing(t) {
    return this.isMissing(r(this.max, this.lastMinute(t)));
  }
  divideByStep(t) {
    return M(t.getMinutes(), this.min.getMinutes()) / this.step;
  }
  lastMinute(t) {
